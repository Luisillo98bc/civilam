...
export default function Footer() {
  ...
  return (
    <footer className="bg-[#030712] text-[#E2E8F0] pt-20 relative overflow-hidden border-t border-white/5">
      ...
      <div className="site-wrapper relative z-10" data-reveal="reveal-scale">
        <div className="grid grid-cols-2 gap-x-5 gap-y-10 pb-14 sm:gap-8 sm:pb-16 lg:grid-cols-2 xl:grid-cols-[1.25fr_.8fr_2.1fr_1.1fr_1.25fr] xl:items-start xl:gap-5">
          {/* Column 1: Brand info */}
          <div className="col-span-2 flex flex-col items-center text-center sm:items-start sm:text-left xl:col-span-1">
            <div className="flex items-center gap-3 mb-4">
              <div className="flex size-12 items-center justify-center rounded-lg bg-[#fbfaf7] p-1.5 shadow-[0_8px_24px_rgba(0,0,0,.2)] ring-1 ring-[#e5a72a]/35">
                <Image src="/logo.png" alt="CIVILAM Logo" width={40} height={40} className="size-9 object-contain" />
              </div>
              <div className="flex flex-col leading-[1.1]">
                <span className="brand-name text-[#F8FAFC]">CIVILAM</span>
                <span className="brand-tagline text-[#94A3B8] opacity-70">INGENIERÍA Y CONSTRUCCIÓN</span>
              </div>
            </div>
          ...
          <div className="col-span-2 flex flex-col items-center text-center sm:col-span-1 sm:items-start sm:text-left xl:col-span-1">
          ...
          <div className="col-span-2 flex min-w-0 flex-col items-center text-center sm:col-span-2 xl:col-span-1">
            <h4 className="mb-5 text-center text-base font-bold tracking-[0.01em] text-[#F8FAFC]">Nuestros Servicios</h4>
            <ul className="grid w-full grid-cols-2 gap-x-4 gap-y-2.5 text-center">
          ...
          <div className="col-span-2 flex flex-col items-center text-center sm:col-span-1 sm:items-start sm:text-left xl:col-span-1">
            ...
            <ul className="flex flex-col items-center gap-4 sm:items-start">
              <li className="flex items-start justify-center gap-3 text-[0.85rem] text-[#94A3B8] sm:justify-start">
          ...
          <div className="col-span-2 flex flex-col items-center text-center sm:col-span-1 sm:items-start sm:text-left xl:col-span-1">
            ...
            <form className="relative" onSubmit={handleNewsletter} aria-label="Suscripción al boletín">
              ...
              <label htmlFor="newsletter-email" className="sr-only">Correo electrónico para el boletín</label>
              <input id="newsletter-email" type="email" name="email" placeholder="Tu correo electrónico" required className="w-full bg-white/5 border border-white/10 py-3 pl-4 pr-12 rounded-[10px] text-[#F1F5F9] font-inherit text-[0.85rem] outline-none transition-all duration-300 placeholder:text-[#64748B] focus:border-accent-red focus:shadow-[0_0_0_3px_var(--accent-red-glow)] disabled:opacity-50 disabled:cursor-not-allowed" disabled={newsletterStatus === 'sending'} />
            ...
            {newsletterStatus === 'success' && (
              <p className="mt-2 text-[0.8rem] text-[#4ade80] font-medium animate-[fadeIn_0.3s_ease]" role="status" aria-live="polite">✓ ¡Gracias por suscribirte!</p>
            )}
            {newsletterStatus === 'error' && (
              <p className="mt-2 text-[0.8rem] text-[#f87171] font-medium animate-[fadeIn_0.3s_ease]" role="alert">✗ Error al suscribirse. Intenta de nuevo.</p>
            )}
          </div>
        </div>
      </div>
      ...
      {/* Bottom Bar */}
      <div className="site-wrapper" aria-hidden="true">
        <div className="h-px bg-gradient-to-r from-transparent via-[#e5a72a]/50 to-transparent" />
      </div>
      <div className="bg-[#070D18] py-5 text-[0.75rem] text-[#64748B]">
