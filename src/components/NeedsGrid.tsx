    ...
    <section className="section bg-[#f4f2ed]">
      <div className="site-wrapper">
        <div className="grid gap-8 lg:grid-cols-[.72fr_1.28fr] lg:gap-20">
          ...
          <div className="border-t border-slate-300" data-reveal>
            {needs.map(([number, title, description, href]) => (
              <Link key={number} href={href} className="group grid gap-4 border-b border-slate-300 py-7 transition hover:bg-white/70 sm:grid-cols-[48px_1fr_36px] sm:px-4" data-reveal-delay={((Number(number) - 1) % 5) + 1}>
